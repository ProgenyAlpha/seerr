...
import { messages as parentalControlsMessages } from '@app/components/UserProfile/UserSettings/UserParentalControlsSettings';
...
const messages = defineMessages('components.UserList', {
  ...
  variesacrossusers: 'Varies across selected users',
});

type ParentalControlField = 'maxMovieRating' | 'maxTvRating' | 'blockUnrated';
...
const BulkEditModal = ({
  ...
  const [touchedFields, setTouchedFields] = useState<
    Set<ParentalControlField>
  >(new Set());
  // Mirrors touchedFields synchronously so the in-flight fetch below can
  // check the latest touched state at the moment it resolves, rather than
  // the stale state captured when the fetch started. Without this, an edit
  // made while the fetch is in flight gets clobbered when the fetch
  // resolves and unconditionally applies the server values.
  const touchedFieldsRef = useRef<Set<ParentalControlField>>(new Set());

  const [isLoadingParentalControls, setIsLoadingParentalControls] =
    useState(true);

  const blockUnratedRef = useRef<HTMLInputElement>(null);

  const markTouched = (field: ParentalControlField) => {
  ...
  const fetchParentalControls = useCallback(async () => {
    if (!selectedUserIds.length) {
      setIsLoadingParentalControls(false);
      return;
    }

    setIsLoadingParentalControls(true);
    ...
    try {
      const results = await Promise.allSettled(
        selectedUserIds.map((id) =>
          axios.get<UserSettingsParentalControlsResponse>(
      ...
      const settings = results
        .filter(
          (
            ...
            AxiosResponse<UserSettingsParentalControlsResponse>
          > => r.status === 'fulfilled'
        )
        .map((r) => r.value.data);
          ...
          <div className="form-row">
            ...
            <div className="form-input-area">
              <div className="form-input-field">
                <select
                  id="maxMovieRating"
                  disabled={isLoadingParentalControls}
                  value={
                    showMixedMovieHint ? '__mixed__' : currentMaxMovieRating
                  }
                  onChange={(e) => {
                ...
                >
                  {showMixedMovieHint && (
                    <option value="__mixed__" disabled>
                      {intl.formatMessage(messages.variesacrossusers)}
                    </option>
                  )}
                  <option value="">
                    {intl.formatMessage(parentalControlsMessages.norestriction)}
                  </option>
            ...
            <div className="form-input-area">
              <div className="form-input-field">
                <select
                  id="maxTvRating"
                  disabled={isLoadingParentalControls}
                  value={showMixedTvHint ? '__mixed__' : currentMaxTvRating}
                  onChange={(e) => {
              ...
              <div className="mt-2 text-sm text-gray-400">
                {intl.formatMessage(
                  parentalControlsMessages.maxtvratingUnratedWarning
                )}
              </div>
            </div>
          </div>
